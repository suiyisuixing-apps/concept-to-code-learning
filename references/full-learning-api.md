...
The paired smoke also compares this packaged client with a direct HTTP request using the same frozen context, question, level, source scope and provider doubles. It checks identical content and source hashes and no implicit note save. This is transport parity only: no real model, host activation or teaching-quality improvement is measured. The product checkout's docs/delivery/lead/SKILL_EVALUATION.md records the separate real-model evaluation gate and protocol.
