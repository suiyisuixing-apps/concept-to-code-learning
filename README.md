...
当前为 **0.2.0 开发候选**。基础整合已通过 PR #69 进入 main；本分支另外包含 PR #75–#77 的代码库工作台和 Windows 修补，这些改动仍待合并。历史模型案例见 [2026-09-11 审核记录](docs/delivery/lead/FINAL_REVIEW.md)，本轮修补范围与验收边界见 [审计修正](docs/delivery/lead/REVIEW_CORRECTIONS.md)。引用、格式和 CI 通过不等于模型讲解每一句都正确。
...
4. 选择实际可用的模型和来源模式。「GitHub 找代码」会根据问题与上下文提取技术词；可选填关键词限定范围。「指定仓库」使用 `owner/repo`，本地模式只展示预先授权的仓库，「仅文档」不检索代码。也可切到「代码库」，按文件夹打开公开仓库的固定版本源码，在右边提问。
5. 点击「发送」，核对来源卡上的固定 Commit、文件、行号、许可与原始代码。追问保留对话，话题或范围变化时重新检索；搜索设置中可以比较两个仓库。
