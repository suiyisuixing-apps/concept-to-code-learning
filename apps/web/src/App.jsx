      ...
      <section className="assistant pane" aria-busy={busy || contextBusy}>
        ...
        {tab === "chat" ? <>
          ...
          <div className="composer">
            ...
            {advanced && <div className="search-settings">{scopeMode === "public_search" && <><input aria-label="批准的搜索词" maxLength={500} value={terms} onChange={(e) => changeScope(() => setTerms(e.target.value))} placeholder="搜索知识点（可选）"/><small>公开搜索只使用通用知识点；填写后将按这些词搜索。</small></>}<label className="check"><input type="checkbox" checked={compare} onChange={(e) => changeScope(() => setCompare(e.target.checked))}/>比较两个仓库</label></div>}
