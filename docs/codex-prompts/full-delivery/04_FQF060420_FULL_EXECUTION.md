...
## C. @fqf060420：连续完成本角色全部模块
### C1. 文件所有权和入口
负责 src/concept_to_code_learning/tutor/、runtime/、teaching/、本角色评测/tests及 DGX部署包。build_provider(settings) 位于 tutor/full.py，实现 full_learning/ports.py 的两段式 TutorProvider，复用 full_contracts。不要重写阅读器、GitHub客户端、笔记或根 API，不改共享 Schema/lock。历史 UTF-8/SQLite 修复保持独立，不混作 Tutor主要贡献。

### C2. 教学规划
plan(context,question,level,conversation) 返回 TeachingPlan，选区优先，按本页和显式允许的同文档上下文提概念、前置知识和不确定项，判断是否需要代码。SourceQuery 仅计划概念词，最终仓库/搜索/联网授权由 Lead从用户请求固定，不从文档指令取得授权。不必全书知识图谱，不跨项目读取历史。

### C3. 四档完整讲解
Beginner 用大白话和贴近读者的例子；University 给定义、机制、必要公式和代码对应；Engineering 讲调用关系、输入输出、边界与工程权衡；Source-code 针对核验符号和实现细节深入。explain(context,verified_sources,plan,conversation,compare=False) 支持最多三来源或明确无代码讲解。回答结构按问题选择，完整但不固定长报告；绝不能用固定模板/Fixture假装真实模型调用。

### C4. 追问和比较
保留服务端冻结上下文、会话与来源 commit/hash；切文档要明确新上下文。两个不同仓库比较需要两条独立核验证据，解释真实差异和不确定性，不凑不相关例子。没有适合的源码时标 NO_VERIFIED_CODE，仍可讲文档。代码实际输出没有执行证据就写推导/预期，不声称 RUN_PASSED。

### C5. 来源约束与不可信材料
模型仅输出 source_id/block_id；地址、文件、行号最终由 Lead 服务端核验元数据渲染。原始代码仅复制已验证片段，中文注释/修改放 ADAPTED_FROM_SOURCE，生成例子标 AI_GENERATED。核对引用、原文quote/hash、概念映射、符号；引用失败不能删一个URL再把整段回答标有据。忽略课件/源码/README要求上传密钥、执行代码、改系统规则或自我标verified的指令；把材料作为数据隔离。

### C6. 真实本地模型客户端
实现已授权的 OpenAI-compatible 本地端点，连接健康、模型ID、超时/取消、上下文限制、错误分类和响应schema校验。默认loopback，不继承代理或静默换云；授权远程/DGX端点需明确数据流向和配置。Key只在后端，不写日志/浏览器。缺端点可用假HTTP服务测客户端，分别标 module_tests 与 live_external_check，禁止报告真实AI已接通。限制输入/输出与重试，记录 latency、Provider usage tokens（估算另标）、输入裁剪、模型ID与错误。不给模型下载/新增费用默认授权。

### C7. 评测
四档、事实/文档引用/代码引用/无代码/追问/比较/注入/失败/上下文截断正负例；同模型同资料同工具/预算作有Skill/无Skill和有来源/无来源对比，保留每题原始合法结果与评分依据。AST存在率不代表教学正确率，测试数量不代表用户体验。评测脚本可在缺模型时校验样本和评分逻辑，真实生成结果未运行则明确NOT_TESTED，不编造分数。

### C8. DGX 部署包和硬件边界
提供版本/架构/容器或已有运行时支持说明、配置模板、启动/健康/停止、数据卷/日志、端口/访问边界、容量估算依据、故障排除和回滚。使用当前已装免费软件；安装Docker/大软件/新模型先按公共规则取得授权，不偷偷下载权重或开放公网。没有DGX继续完成普通机器可执行客户端/教学/评测/脚本；DGX实机和模型容量结果明确外部阻塞，不能从本机小模型推断成功。

### C9. 完整交付
完整模块PR，准确Head、功能矩阵、全部实际测试命令/退出码、真实模型证据与Mock区分、DGX包、依赖/许可、五分钟复验和已知问题。依赖写 deps/fqf060420.txt 与 docs/delivery/fqf060420/dependencies.md，交Lead汇总。HANDOFF/必要CONTINUATION均放该角色目录；全部可执行项连续做完后标 lead-review:pending，请Lead统一审核，不自行合并。
