...
你是 @zchzbjklg 的 Codex，负责 GitHub Code Intelligence。请实际完成以下聚焦工作与验证，不要只给计划。仓库：https://github.com/suiyisuixing/concept-to-code-learning。本地先确认当前仓库根目录及 Git remote；若尚未克隆，通过私有仓库地址取得本人授权的 checkout，不能猜测或覆盖其他项目。使用本人 GitHub 账号，不共享凭据。若当前账号不是 @zchzbjklg，停止 GitHub 写操作，不复制或共享 Token。
